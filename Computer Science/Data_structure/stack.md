...
- 문자열 수식 계산의 일반적인 방법
  ...
  - 중위 표기법 -> 후위 표기법 변환법 (손으로 계산)

    - 수식의 각 연산자에 대해서 우선 순위에 따라 괄호로 다시 표현
    - 각 연산자를 그에 대응하는 오른쪽괄호 뒤로 이동
    - 괄호 제거

  - 중위 표기법 -> 후위 표기법 변환법 (스택이용 - 순차적)

    - 중위 표기법의 수식을 하나씩(토큰) 읽어오기

    - 토큰이 연산자면 스택의 top과 비교하여 

      - (우선순위가 높으면 or 여는 괄호면 or 비어있으면) push

      여는 괄호의 우선순위는 밖에 있을 땐(icp) 가장 높지만, 스택에선(isp) 가장 낮으므로

      - 우선순위가 높지 않으면(같은 경우 포함), 낮은 연산자를 만날 때까지 pop 

        -> top이 토큰보다 우선순위가 낮은 연산자면 push

    - 토큰이 닫는 괄호`)`면

      - 여는 괄호`(`를 만날때까지 모두 pop
      - 닫는 괄호, 여는 괄호는 버림

    - 토큰이 피연산자면 그대로 출력(저장)

    - 수식 끝났을 때, 스택이 비어있으면 정상 종료 -> 결과 값(후위표기법 수식)

    - **우선순위** `*`, `/` : isp 2, `+`, `-` : isp 1 , `(` : isp 0

  - 후위 표기법 수식 계산 (스택 이용)

    - 피연산자를 만나면 스택에 push
    - 연산자를 만나면, 필요한만큼의 피연산자를 pop하여 연산
    - 연산결과를 다시 스택에 push
    - 수식이 종료되면, 마지막으로 pop하여 출력
