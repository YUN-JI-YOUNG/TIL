...
- 논리곱 `AND`, `∧`    
   - p,q가 명제일 때, p,q 모두 참인 경우 참     
- 논리합 `OR`, `∨`     
   - p,q가 명제일 때, p,q 모두 거짓인 경우 거짓 = 1개 이상이 참이면 참     
- 배타적 논리합 `XOR`       
   - p,q가 명제일 때, p,q 가 서로 다른 경우 참 = 둘 중 1개만 참이면 참     
- **연산자 우선순위**    
   - `NOT` > `논리 연산자` > `->`, `<->`      
   - 연산자 간의 우선순위가 존재하긴 하지만 괄호 활용하는 것을 추천       
 </br>    
 
- 항진명제 : 진릿값이 항상 참   
- 모순명제 : 진릿값이 항상 거짓  
- 사건명제 : 항진명제,모순명제가 아닌 명제     
- 조건명제(`p->q`) : 명제 p가 조건(원인), q가 결론(결과)로 제시되는 명제     
   - `T -> F` 인 경우에만 `F`         
- 쌍방조건명제(`p<->q`) : 명제 p,q가 모두 조건이면서 결론인 명제     
   - 조건명제와 동일하게 T->F인 경우에만 F 이므로, `T<->F, F<->T` 인 경우에만 `F`     

**ex**   
'내가 복권에 당첨되면, 자동차를 산다.' 라는 조건명제에서     
...
- Trivial Proof(자명한 증명)    
   - 모든 x에 대해, p(x) -> q(x)를 증명할 때, q(x)가 항상 참인 경우 (T)       
   - `실수 x에대해, x < 0이면, x^2 + 1 >0 이다` : 실수 x에 대해 x^2 > -1 는 항상 참이므로 q(x)는 항상 참 = 명제도 참임을 증명    
- Vacuous Proof(공허한 증명)
   - 모든 x에 대해, p(x) -> q(x)를 증명할 때, p(x)가 항상 거짓인 경우 (T)    
