...
### 네트워크 공격 기법     
    - 랜드 어택(Land Attack)    
        - 공격자가 패킷의 출발 주소나 포트를 임의로 변경하여 출발지와 목적지의 주소를 동일하게 하여     
          컴퓨터의 실행 속도를 느리게 하거나 동작을 마비시켜 서비스 거부 상태에 빠지도록 하는 공격 기법      

    - 서비스 거부 공격 (DOS)     
        - 표적이 되는 서비스 자원 고갈 목적     
        - 다수의 공격자 / 시스템에서 대량의 데이터를 한 곳의 서버에 집중 전송하여 서버의 정상적인 기능 방해       
        - DDOS와의 차이는 attacker가 직접 공격 수행 

    - 분산 서비스 거부 (DDOS)    
        - attacker 가 여러 대의 컴퓨터를 감염시켜 동시에 타깃 시스템 집중 공격     
        - 짧은 시간안에 서버 마비 가능       
        => DOS보다 치명적이고 DOS와의 차이는 attacker가 감염시킨 좀비 pc가 공격 수행      

    - Ping of Death    
        - ping 명령을 전송할 때 패킷의 크기를 인터넷 프로토콜 허용 범위 이상으로 전송하여 네트워크 마비시킴

    - 스머핑   
        - IP / ICMP의 특성을 악용하여 엄청난 양의 데이터를 한 사이트에 집중 전소하여 네트워크 불능 상태로 만듦

    - SYN Flooding   
        - 공격자가 가상의 클라이언트로 위장하여 3-way-handshake 과정을 의도적으로 중단시킴       
        => 서버가 대기 상태에 놓여 정상적으로 서비스 수행 불가    

    - TearDrop     
        데이터의 송,수신 과정에서 패킷의 크기가 여러 개로 분할되어 전송할 때,         
        분할 순서를 변경시켜 수신측에서 패킷을 재조립할 때 오류로 인한 과부하 발생 => 시스템 다운    
...
## SW 개발 보안    
  ...
  - MD5     
    - RFC 1321 로 지정되어 있으며, 주로 프로그램이나 파일이 원본 그대로 인지를 확인하는 무결성 검사에 사용     
    - 1991년 로널드 라이베스트가 예전에 쓰이던 MD4를 대체하기 위해 고안된 128비트 암호화 해시 함수      
 ...
 - 식별자 표기법 
  ...
  - 비선점형 프로세스 스케줄링     
      - HRN(Highest Response ratio Next)     
          - SJF의 약점인 긴 작업과 짧은 작업 사이의 불평등을 보완하기 위한 방법       

          - 우선순위 계산식을 통해 우선순위 결정     
              (대기 시간 + 서비스 시간) / 서비스 시간    
              => 시스템 응답 시간이 커질수록 우선순위가 높아짐    

      - SJF (Shortest Job First)    
        - 프로세스의 실행시간이 가장 짧은 프로세스를 먼저 실행    

      - FIFO (First In Frist Out)    
        - 가장 먼저 들어왔던 프로세스를 먼저 실행 (대기시간이 가장 ↑)    

      - 우선순위   
        - 미리 정의한 알고리즘대로 프로세스 우선순위 결정    
