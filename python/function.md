...
#### nonlocal   
- 전역(global) 제외하고 가장 가까운 scope의 변수 연결   
- 이름 공간 내에 존재하는 변수(= 이미 정의된 이름)과의 연결만 가능   


#### 변수 접근 절차(LEGB)      
1. 함수 스코프 내에서 가장 먼저 탐색(Local scope) 　　　　　2. 해당 함수의 상위 함수 내에서 탐색(Enclosed scope)    
3. 함수 밖, 전역 스코프에서 탐색(Global scope) 　　　　　 4. 파이썬에 내장되어있는 속성 탐색(Builtin scope)          
-> 지역변수와 전역변수의 이름이 같으면, 지역변수를 먼저 탐색하므로 전역변수에 접근 못할 수도 있음.          
-> 접근하고자 하는 전역변수 앞에 global 기재        
-> 지역 변수는 함수 종료되면 사라지므로 함수 밖(전역)에서 함수 내의 변수(지역 변수)는 탐색 불가      
-> 함수 내에선 함수 밖의 변수(전역 변수)에 접근할 수 있지만 수정은 불가       

###### 변수 앞에 global과 nonlocal을 붙여 정의하는 경우, 같은 코드 블록에선 해당 이름이 그 전에 나올 수 없음     
...
### 2-4. 재귀함수   
###### 자기 자신을 호출하는 함수   
- 최소 1개의 베이스 case가 존재하며, 해당 case에 도달하면 재귀 함수가 종료     
- 재귀함수로 만들기 쉬운 공식은 `팩토리얼`과 `피보나치 수열`    
- 피보나치 수열은 `a(n) = a(n-1) + a(n-2)` 이고, 팩토리얼은 `a(n) = n * a(n-1)`    
-> 이렇게 점화식으로 표현가능한 수식들은 재귀함수로 표현하면 가독성 향상 가능   
- 함수를 계속 호출하다보니 반복문보단 수행시간 더 길게 소요    
- But, 변수 사용을 줄일 수 있고, 가독성이 좋으며, 점화식으로 표현되는 식들은 로직짜기 수월하다는 장점   
